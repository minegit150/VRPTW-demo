...
async function startSimulation() {
  ...
  showNotification('Đang tính toán tuyến đường tối ưu bằng phương pháp ' + solverMethod + '...');
    // Sử dụng cả 2 cách lấy CSRF token để đảm bảo
    const csrftoken = document.querySelector('[name=csrfmiddlewaretoken]')?.value 
                   || getCookie('csrftoken');
  try {
    const payload = { /* giữ nguyên payload */ };

    // Sửa đường dẫn API cho đúng với urls.py
    const response = await fetch('/api/solve/', { // ĐÃ SỬA TỪ '/vrp/api/solve/'
        method: 'POST',
        headers: {
